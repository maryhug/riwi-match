...
function StatCard({ label, value, detail, icon: Icon, accentColor, loading }: StatCardProps) {
  return (
    <div className="bg-white border border-slate-200 rounded p-4 flex flex-col gap-3">
      ...
      <div>
        <p className="text-[11px] text-slate-400 mb-1.5">{detail}</p>
        <div className="h-0.5 rounded-full bg-slate-100 overflow-hidden">
          <div className="h-full rounded-full" style={{ width: '100%', background: accentColor, opacity: 0.4 }} />
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <StatCard label="Activos"       value={activos}     detail={`${pct(activos)}% del total`}  icon={Briefcase}   accentColor="#7C3AED" loading={isLoading} />
        <StatCard label="En matching IA" value={enMatch}    detail="Procesando CVs"                icon={Zap}         accentColor="#D97706" loading={isLoading} />
        <StatCard label="Completados"   value={completados} detail="Match + profiling"             icon={Trophy}      accentColor="#059669" loading={isLoading} />
        <StatCard label="Total"         value={total}       detail="Todos los periodos"            icon={LayoutGrid}  accentColor="#2563EB" loading={isLoading} />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-4">
        ...
        {/* Bar chart */}
        <div className="lg:col-span-3 bg-white border border-slate-200 rounded">
            ...
            {isLoading ? (
              ...
              <ResponsiveContainer width="100%" height={200}>
                <BarChart data={areaData} margin={{ top: 8, right: 0, left: -28, bottom: 0 }}>
                  ...
                  <RechartsTooltip
                    contentStyle={{ border: '1px solid #E2E8F0', borderRadius: 4, fontSize: 12, boxShadow: 'none' }}
                    cursor={{ fill: '#F5F3FF' }}
                  />
            ...
            {isLoading ? (
              ...
              <div className="divide-y divide-slate-100">
                {Object.entries(processStatusConfig).map(([status, cfg]) => {
                  ...
                  return (
                    <div key={status} className="py-2.5 flex items-center gap-3">
          ...
          <table className="w-full text-sm text-left">
            <thead>
              <tr className="border-b border-slate-100 bg-slate-50">
                <th className="px-4 py-2.5 text-[10px] font-semibold text-slate-400 uppercase tracking-wider">Proceso</th>
      ...
      {/* Banners */}
      {!isLoading && enMatch > 0 && (
        <div className="flex items-start gap-3 p-3 rounded border border-violet-200 bg-violet-50">
          <Sparkles className="w-4 h-4 text-violet-600 shrink-0 mt-0.5" />
          <div>
            <p className="text-sm font-semibold text-slate-900">IA procesando ahora</p>
            <p className="text-xs text-slate-600 mt-0.5">
              Tienes <span className="font-bold text-violet-700">{enMatch} proceso{enMatch > 1 ? 's' : ''} en match</span> ejecutándose.
            </p>
          </div>
        </div>
      )}
      ...
      {!isLoading && total === 0 && (
        <div className="flex items-start gap-3 p-3 rounded border border-blue-200 bg-blue-50">
          ...
          <div>
            ...
            <p className="text-xs text-slate-600 mt-0.5">
              Ve a{' '}
              <Link href="/hiring-processes/new" className="font-semibold text-blue-600 hover:underline">Crear proceso</Link>
              {' '}para comenzar.
            </p>
          </div>
        </div>
