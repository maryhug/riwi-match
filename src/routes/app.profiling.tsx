...
function Profiling() {
  ...
  const cancelMutation = useMutation({
    ...
    onError: (err: unknown) => toast.error(err instanceof Error ? err.message : "No se pudo cancelar"),
  });

  const runs = data?.profiling_runs ?? [];
  const cola = runs.filter((r) => r.status === "QUEUED" || r.status === "PENDING").sort((a, b) => a.created_at.localeCompare(b.created_at));
  const activas = runs.filter((r) => r.status === "CALLING" || r.status === "ANSWERED");
  const completadas = runs.filter((r) => r.status === "COMPLETED");
  const fallidas = runs.filter((r) => ["NO_ANSWER", "FAILED", "RETRY_PENDING", "VOICEMAIL_DETECTED", "CANCELLED"].includes(r.status));

  const completadasHoy = completadas.filter((r) => isToday(r.completed_at)).length;
  const contactables = completadas.length + fallidas.filter((r) => ["NO_ANSWER", "FAILED", "VOICEMAIL_DETECTED"].includes(r.status)).length;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { l: "Llamadas activas", v: `${activas.length} / ${MAX_CONCURRENT_CALLS}`, cn: "bg-primary text-primary-foreground" },
          { l: "En cola", v: cola.length.toString(), cn: "bg-indigo-500 text-white" },
          { l: "Completadas hoy", v: completadasHoy.toString(), cn: "bg-success text-success-foreground" },
          { l: "Tasa de contacto", v: contactables > 0 ? `${tasaContacto}%` : "—", cn: "bg-warning text-warning-foreground" },
        ].map((k) => (
      ...
      <div className="glass rounded-2xl p-3 text-xs text-muted-foreground flex flex-wrap gap-2 items-center">
        <span className="px-2 py-1 rounded-md bg-accent text-accent-foreground font-semibold">Máx {MAX_CONCURRENT_CALLS} simultáneas (RB-005)</span>
        <span className="flex-1" />
        <span>Cada llamada informa que es un asistente automatizado y solicita consentimiento.</span>
      </div>

      {isLoading ? (
        <div className="py-16 text-center text-sm text-muted-foreground">Cargando llamadas…</div>
      ) : runs.length === 0 ? (
        <div className="py-16 text-center text-sm text-muted-foreground">No hay llamadas de profiling activas.</div>
      ) : (
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-4">
          <Column title="En cola" count={cola.length} accent="info" icon={ListTodo}>
            {cola.map((r, i) => (
              <GlassCard key={r.id} className="p-3 border border-indigo-500/20 bg-indigo-500/5 h-[80px] flex flex-col justify-between">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2.5 min-w-0">
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="text-[10px] text-muted-foreground">Intento {r.call_attempts + 1}</div>
                    </div>
                  </div>
                  <button
          ...
          <Column title="En llamada" sub={`máx. ${MAX_CONCURRENT_CALLS}`} count={activas.length} accent="primary" icon={PhoneCall}>
            {activas.map((r) => (
              <GlassCard key={r.id} className="p-3 border border-primary/30 bg-primary/5 h-[90px] flex flex-col justify-between">
                <div className="flex items-center gap-2.5">
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="text-sm font-semibold truncate">{r.candidate_name}</div>
                    <div className="text-[10px] text-muted-foreground">Intento {r.call_attempts}</div>
                  </div>
                </div>
          ...
          <Column title="Completadas" count={completadas.length} accent="success" icon={CheckCircle2}>
            {completadas.map((r) => (
              <GlassCard key={r.id} className="p-3 border border-emerald-500/20 bg-emerald-500/5 h-[90px] flex flex-col justify-between">
                <div className="flex items-center gap-2.5">
                  <CheckCircle2 className="h-5 w-5 text-emerald-500 shrink-0" />
                  <div className="min-w-0 flex-1">
                    <div className="text-sm font-semibold truncate">{r.candidate_name}</div>
                    {r.advancement_probability && <div className="text-[10px] text-muted-foreground">Avance: {r.advancement_probability}</div>}
                  </div>
                </div>
                <button onClick={() => setModalRun(r)} className="text-[10px] font-semibold text-primary hover:underline text-left">Respuestas</button>
              </GlassCard>
            ))}
          </Column>

          <Column title="No contestadas / Fallidas" count={fallidas.length} accent="destructive" icon={XCircle}>
            {fallidas.map((r) => (
              <GlassCard key={r.id} className="p-3 border border-rose-500/20 bg-rose-500/5 h-[90px] flex flex-col justify-between">
                <div className="flex items-center gap-2.5">
                  <XCircle className="h-5 w-5 text-rose-500 shrink-0" />
                  <div className="min-w-0 flex-1">
                    <div className="text-sm font-semibold truncate">{r.candidate_name}</div>
                    <div className="text-[10px] text-muted-foreground">{r.status === "VOICEMAIL_DETECTED" ? "Buzón de voz" : r.status === "CANCELLED" ? "Cancelada" : ""}</div>
                  </div>
                </div>
...
function Column({ title, sub, count, accent, icon: Icon, children }: {
  title: string; sub?: string; count: number; accent: "primary" | "info" | "success" | "destructive";
  icon: React.ElementType; children: React.ReactNode;
}) {
  const accentMap: Record<string, string> = {
    primary: "bg-primary", info: "bg-indigo-500", success: "bg-success", destructive: "bg-destructive",
  };
  return (
