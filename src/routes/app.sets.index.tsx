...
function Sets() {
  ...
  const { data, isLoading } = useQuery({ queryKey: ["question-sets"], queryFn: () => getQuestionSets() });
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">Plantillas de profiling automatizado.</p>
        </div>
      ...
      {isLoading ? (
        ...
        <div className="py-16 text-center text-sm text-muted-foreground">No hay sets de preguntas. Crea uno para configurar profiling.</div>
      ) : (
            ...
            <GlassCard key={s.id} className="cursor-pointer" onClick={() => navigate({ to: "/app/sets/$id", params: { id: s.id } })}>
              <div className="flex items-start justify-between mb-3">
                ...
                <div className="flex items-center gap-1">
                  <span className="px-2 py-0.5 rounded text-[10px] font-bold bg-accent text-accent-foreground">v{s.version}</span>
                  <span className={`px-2 py-0.5 rounded text-[10px] font-semibold ${STATUS_STYLE[s.status]}`}>{QUESTION_SET_STATUS_LABEL[s.status]}</span>
                </div>
              </div>
              <h3 className="font-semibold leading-tight">{s.name}</h3>
              {s.description && <p className="text-xs text-muted-foreground mt-1 line-clamp-2">{s.description}</p>}
              <div className="mt-4 text-xs text-muted-foreground">{s.questions?.length ?? 0} pregunta{(s.questions?.length ?? 0) === 1 ? "" : "s"}</div>
              {s.status === "ACTIVE" && (
