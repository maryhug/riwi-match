...
import { Clock, Headphones, MessageSquare, PhoneCall, ShieldAlert } from "lucide-react";
    ...
    <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="max-w-5xl max-h-[85vh] overflow-y-auto">
        ...
        <div className="grid md:grid-cols-2 gap-5 text-sm">
          {/* ── Columna izquierda: resumen, respuestas, costos ── */}
            ...
            <div className="rounded-xl border border-border bg-background/40 p-4 space-y-2">
              ...
              {run.has_audio && (
                <div className="flex items-center gap-2 text-xs text-muted-foreground pt-1">
                  ...
                  <audio controls src={`/dl/profiling-audio/${run.id}`} className="h-8 w-full" />
                </div>
              )}
            </div>
          ...
          <div className="min-w-0">
            <div className="text-xs font-semibold uppercase tracking-wider text-muted-foreground mb-2 flex items-center gap-1.5">
              <MessageSquare className="h-3.5 w-3.5" /> Conversación completa
            </div>
            {turns.length === 0 ? (
              <div className="text-xs text-muted-foreground py-8 text-center rounded-xl border border-dashed border-border">
